...
STATUS=`aws elasticbeanstalk describe-environment-health --environment-name "$EB_ENV_NAME-$CIRCLE_BRANCH" --attribute-names All | sed  -n '/"Status":/p'`
...
aws elasticbeanstalk update-environment --region=eu-west-1 --environment-name "$EB_ENV_NAME-$CIRCLE_BRANCH" --version-label `git rev-parse --short HEAD`
