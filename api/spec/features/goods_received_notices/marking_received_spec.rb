feature 'Marking Purchase Order as received', booking_db: true do
  ...
  scenario 'Marking PO as received when already received' do
    when_a_received_purchase_order_is_received
    then_grn_received_units_should_not_change
  end
  ...
  def when_a_received_purchase_order_is_received
    grn_event.received = true
    page.driver.post goods_received_notice_path(grn), { _method: 'patch',
                                                        goods_received_notice: grn_attrs }
  end

  def then_grn_received_units_should_not_change
    expect(subject['units_received']).to eq(grn.goods_received_notice_events.first.units)
    expect(subject['cartons_received']).to eq(grn.goods_received_notice_events.first.cartons)
  end
  ...
  let(:grn_attrs) do
    { goods_received_notice_events_attributes: [{ id: grn_event.id, received: true }] }
  end
end
