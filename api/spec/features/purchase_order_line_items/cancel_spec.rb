feature 'Cancelling purchase orders' do
  ...
  def when_i_cancel_a_line_item
    page.driver.post cancel_purchase_order_line_items_path(id: PurchaseOrderLineItem.first)
  end

  def then_the_line_item_should_be_cancelled
    expect(PurchaseOrderLineItem.first).to be_cancelled
    expect(PurchaseOrderLineItem.second).to_not be_cancelled
  end

  def when_i_cancel_multiple_line_items
    page.driver.post cancel_purchase_order_line_items_path(id: [PurchaseOrderLineItem.first, PurchaseOrderLineItem.second])
  end

  def then_the_line_items_should_be_cancelled
    expect(PurchaseOrderLineItem.first).to be_cancelled
    expect(PurchaseOrderLineItem.second).to be_cancelled
    expect(PurchaseOrderLineItem.third).to_not be_cancelled
  end

  def when_i_cancel_an_entire_purchase_order
    page.driver.post cancel_purchase_order_path(id: PurchaseOrderLineItem.first.po_number)
  end
  ...
  def assert_entire_po_order_to_be_cancelled
    items = PurchaseOrderLineItem.where(po_number: PurchaseOrderLineItem.first.po_number)
    items.each do |item|
  ...
  def assert_every_other_po_not_cancelled
    not_cancelled = PurchaseOrderLineItem
      .where.not(status: -1)
      .where.not(po_number: PurchaseOrderLineItem.first.po_number)
